...
-- 1. Название компании заказчика (company_name из табл. customers) и ФИО сотрудника, работающего над заказом этой компании
-- (см таблицу employees),
-- когда и заказчик и сотрудник зарегистрированы в городе London, а доставку заказа ведет компания United Package (company_name
-- в табл shippers)
SELECT company_name AS customer, CONCAT(employees.first_name, ' ', employees.last_name) AS employee
FROM  orders
INNER JOIN customers USING(customer_id)
INNER JOIN employees USING(employee_id)
WHERE customers.city IN ('London') AND employees.city IN ('London')  AND orders.ship_via=2
...
FROM products
WHERE EXISTS
(SELECT DISTINCT product_id FROM order_details WHERE products.product_id=order_details.product_id AND quantity=10)